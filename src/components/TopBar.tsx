...
function PlayerRow({
  ...
  return (
    <div
      className={`flex items-center gap-2 py-1.5 px-2 rounded-lg ${isCurrent ? "bg-gray-800" : ""} ${!p.alive ? "opacity-30" : ""}`}
    >
      <span className="relative flex-shrink-0">
        ...
        />
        {isCurrent && (
          <span className="absolute -inset-0.5 rounded-full ring-2 ring-yellow-400 pointer-events-none" />
        )}
      </span>
      ...
      {p.alive ? (
        <>
          <span className="text-yellow-300 text-xs font-bold ml-auto">
            {gold}💰
          </span>
          <span
            className={`text-[11px] font-medium w-10 text-right ${net >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {net >= 0 ? "+" : ""}
            {net}/t
          </span>
...
export default function TopBar({
  ...
  const btnBase =
    "px-2.5 py-1 rounded-lg border text-xs font-medium transition-colors border-gray-700 text-gray-400 hover:text-white hover:border-gray-500 bg-gray-900";
    ...
    <div className="relative flex-shrink-0">
      ...
      <div className="hidden md:flex items-center h-11 px-4 bg-gray-950 border-b border-gray-800 gap-3 text-sm">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <img src="/favicon.png" alt="ANTIYOY" className="w-5 h-5" />
        ...
        <div
          className={`flex items-center gap-1.5 flex-shrink-0 transition-all ${isPreview ? "ring-1 ring-yellow-500/50 px-1.5 rounded-lg" : ""}`}
        >
          <span
            className="w-2.5 h-2.5 rounded-full flex-shrink-0"
            style={{ backgroundColor: displayPlayer?.color ?? playerColor }}
          />
          {isPreview && (
            <span className="text-yellow-500/80 text-[9px] font-bold uppercase tracking-wide">
              enemy
            </span>
          )}
          ...
          <span
            className={`text-xs font-medium ${displayNet >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {displayNet >= 0 ? "+" : ""}
            {displayNet}/t
          </span>
        ...
        <button onClick={onOpenRules} className={btnBase} title="Game Rules">
          Rules
        </button>
      ...
      {/* ══ MOBILE BAR ══ */}
      <div
        className="flex md:hidden items-center h-11 px-3 bg-gray-950 border-b border-gray-800 gap-2"
        ref={menuRef}
      >
        {/* Logo + Turn */}
        ...
        >
          ...
          />
          {isPreview && (
            <span className="text-yellow-500/80 text-[9px] font-bold">👁</span>
          )}
          <span className="text-yellow-300 font-bold text-xs">
            {displayGold}💰
          </span>
          <span
            className={`text-[10px] font-medium ${displayNet >= 0 ? "text-green-400" : "text-red-400"}`}
          >
            {displayNet >= 0 ? "+" : ""}
            {displayNet}
          </span>
